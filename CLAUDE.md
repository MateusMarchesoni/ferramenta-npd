...
4. **Regras de dependência entre módulos:** `ingest` não conhece `custo`. `custo` não conhece `escrita`. `ui` não contém regra de negócio. Só `escrita/ooxml.py` toca o arquivo da planilha.
